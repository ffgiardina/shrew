...
protected:
  ...
  RandomVariable<NormalDistribution> grv =
...
TEST_F(NormalDistributionTestFixture, RandomVariableAddition)
{
  ...
  ASSERT_NEAR((grv_a + grv).probability_distribution.Pdf(0),
              0.029732572305907347, 1e-15);
  ASSERT_NEAR((grv + grv_a).probability_distribution.Pdf(0),
              0.029732572305907347, 1e-15);
}
...
{
  auto grv_a = RandomVariable<NormalDistribution>(NormalDistribution(3.0, 1.0));
  ASSERT_NEAR((grv_a - grv).probability_distribution.Pdf(3.0),
              1 / sqrt(2) * 0.3989422804014337, 1e-15);
  ASSERT_NEAR((grv - grv_a).probability_distribution.Pdf(-3.0),
              1 / sqrt(2) * 0.3989422804014337, 1e-15);
}

TEST_F(NormalDistributionTestFixture, ConstantAddition)
{
  ASSERT_NEAR((grv + 2).probability_distribution.Pdf(2.0), 0.3989422804014337,
              1e-15);
  ASSERT_NEAR((2 + grv).probability_distribution.Pdf(2.0), 0.3989422804014337,
              1e-15);
}
...
{
  ASSERT_NEAR((grv * 2).probability_distribution.Pdf(0.0),
              0.3989422804014337 / 2.0, 1e-15);
  ASSERT_NEAR((2 * grv).probability_distribution.Pdf(0.0),
              0.3989422804014337 / 2.0, 1e-15);
}
...
{
  ASSERT_NEAR((grv / 2).probability_distribution.Pdf(0.0),
              0.3989422804014337 * 2.0, 1e-14);
  ASSERT_NEAR((1 / grv).probability_distribution.Pdf(1.0), 0.2419707245191433,
              1e-14);
}
    ...
    namespace kernel {
        ...
        class MaternNoise : public Kernel {
          private: 
            ...
            std::unordered_map<int, double> conditional_idx_to_predictive_noise;
          ...
          public:
            ...
            MaternNoise(std::vector<double> hyperparameters_, MaternSmoothness nu_) {
                ...
                conditional_idx_to_predictive_noise = std::unordered_map<int, double> {};
                nu = nu_;
            };
            MaternNoise(std::vector<double> hyperparameters_, std::vector<double> hp_lower_bounds_, 
            std::vector<double> hp_upper_bounds_, std::vector<int> conditional_indices_, 
            std::vector<int> conditional_indices_noise2_, MaternSmoothness nu_) {
                hyperparameters = hyperparameters_;
                ...
                for (size_t i = 3; i < hyperparameters.size(); ++i) {
                    conditional_idx_to_predictive_noise[conditional_indices_noise2_[i-3]] = hyperparameters[i];
                }  
                nu = nu_;    
                };
        };
